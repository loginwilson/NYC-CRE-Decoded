...
against 0007's tables: reproduction.updates (the phase, lane and workstation rows) and machinery.claims.
...
proven on empty test rows landed directly.
  ...
  5. the two empty rows landed directly -> lane landed +2, phase landed +2 (their registry was already there), and
     host A's own workstation row +2 (migration 0007)
  6. a wrong word is rejected by the cell rule; a registry landed over a registry adds nothing
  7. heartbeat() from both hosts -> two workstation rows in updates, A's carrying its landed count
  8. delete the test rows, their claims and the test workstation rows; reconcile (the counters measured again)
...
def landed(lane, workstation=""):
    r = q("select landed from reproduction.updates where source = 'acris' and lane = %s and workstation = %s", (lane, workstation))
...
assert landed("documentation", "HOST-A") - host_before == 2, "HOST-A's own row should rise by 2"
print("   documentation landed +2, phase landed +2, HOST-A's row +2: OK")
...
print("7. heartbeats from both hosts -> their workstation rows")
...
rows = q("select lane, workstation, workers, landed, last_word, last_seen > now() - interval '1 minute' from reproduction.updates where source = 'acris' and workstation like 'HOST-%' order by lane, workstation")
print("   ", rows)
assert [r[1] for r in rows if r[0] == "documentation"] == ["HOST-A", "HOST-B", "HOST-C"] or [r[1] for r in rows if r[0] == "documentation"] == ["HOST-A", "HOST-B"], rows
assert all(r[2] == 40 and r[5] for r in rows if r[1] in ("HOST-A", "HOST-B") and r[0] == "documentation"), "workers 40 and a fresh last_seen on A and B"
