...
`build_parser` in `rulebook.py` (2069-2097), from its own help text. The commands: `run` (the default), `status`, `stop [lane]`, `width LANE=N`.
...
The cycle lane is `run(roles, args, here)` in the lane part of `rulebook.py` (649-1611): a lane file defines a ROLE - what one
...
|---|---|
| one door | `<lane>.lock` holds the pid: a second start on this machine is refused while the first lives, a stale lock is taken (707-723, 1476-1477). A parked lane (`<lane>.parked`, the reason inside) refuses to start until `--unpark`, which removes the file (1467-1472). A crew has 1 to MAX_WIDTH 128 workers (835, 1473-1475). The cloud must answer before anything is entered (1510-1515); the outbox left from last time lands first (1516) |
| one entry | one pooled session per crew (`make_session`: a pool of MAX_WIDTH + 4 connections, no retries, 878-889), workers born `--stagger` apart on their own thread, one connection each at its birth, keep-alive after (1023-1054); a crew enters when no other crew ON THE SAME DOOR is ramping (a burst of handshakes on one address is the ban condition; crews with no door share the machine's own line and still take turns) and the last crew on any door started its ramp `--entry-gap` ago (`entered_at`, 926, 1024; the gate 1404-1412) - so DIFFERENT doors, being different addresses, enter `--entry-gap` apart and ramp CONCURRENTLY (2026-09-08: eight doors all ramping within a minute, where the old one-ramp-at-a-time rule took ~5 min per door). The wire must be up: no answer from a neutral host = wait a minute, no try spent (818-829, 1414-1417). `--door`, repeatable, makes one crew per role per door (1496-1500), each crew's session through its own proxy (878-889, 897-900), named `<lane>@doorN` in the log; ONE BATCH and doors together are refused at start |
| the exit-pool check | before every entry, on its own thread so the other crews keep feeding and landing (1418-1426): five draws of the public exit (api.ipify.org, one fresh connection each, 1 s apart, 838-857); five answers in one /24 = enter, else wait 30 s and draw again, the heartbeat saying "waiting for a settled exit pool" (860-875); a verdict older than 120 s is drawn again (1429). `--no-pool-check` skips it. No owner lookup, no spent list: the block is the whole check. Through a door the draws go through the crew's tunnel and answer the rented address, one block by construction (838-857) |
| the batch | the feed keeps the queue a batch ahead: `claim()` for `--claim` (default 12 x the width) with `--ttl`, the registries fetched with the ids (1212-1235); the fresh batch is claimed right before the ramp, never before a wait (1431). An empty list = ask again in 60 s; a failed claim = 30 s (1231, 1214) |
| landing | results go to `<lane>.outbox.jsonl` first and to `land()` second, in chunks of 500, so a cloud hiccup loses nothing (586-647, 1238-1252). The flush: as soon as 200 results wait (1541-1544) and on the minute tick (1566) |
| the minute tick | every 60 s (1558): the control file (1560), the role's check - documentation: the drive is still there, or park with exit 6 (1565) - the landing, the PROGRESS line (1567, 1255-1272), the session manager, the quiet counter, the heartbeat with the live worker count (1568-1588) |
| a worker's failures | a fetch error never stops the lane: the document is written to `<lane>.fails.jsonl` and left for a later pass (951-960); a transport error is re-asked once, then failed, and the cut worker pauses HANGUP_PAUSE_S 5 s before it dials again (986-996); a `Retry` (a short document, an unknown page shape) is a fail, not a verdict (997-1001); a worker above the width leaves after its document (1005-1006) |
| the hang-up | the session closed: every worker cut inside HANGUP_WINDOW_S 60 s and nothing landed for HANGUP_QUIET_S 10 s (832-833, 1008-1021; during a ramp the whole width is the workers born). Then at once: leave, land what the crew holds, drop the cut batch (its claims expire on their own), wait `--redial-wait` with no line open, re-enter once on a fresh batch (1325-1354) |
| served or refused | the last re-entry was SERVED when it landed SERVED_LANDINGS 300 or lived SERVED_S 300 s (834, 1322): the incident closes and the wait halves back toward the base (1337-1338); otherwise it was REFUSED and the next wait doubles, cap 4,800 s (1340); `--tries` refused in a row = park, exit 3 (1341-1342). The try is spent at the re-entry itself, never while the wire is down (1432-1435) |
| dead transport | 3 x width transport errors in a row and nothing landed for 60 s = the same hang-up (1552-1554); five minute ticks asking with nothing landing = "our wire", the same hang-up (1578-1583) |
| the wall | 40 consecutive 503/429 (the wall counts these two only) = park, exit 4, not retrying, not rotating (983-984, 1546-1548) |
| the refusal | the source's notice page (`Refused`, raised by the source module's detector) = park at once, exit 2 (978-979). With more than one `--door`, the notice on one door retires that door's crew alone - its lines close, what it fetched lands, its claims expire (1090-1099, 1527-1536) - and the lane parks when the last open door is refused |
| the park | `<lane>.parked` with the reason; the process stops; the heartbeat carries the reason on the way out (1101-1112, 1597-1610). Exit codes: 0 stopped (the control file, `--limit`, Ctrl+C, a signal) · 2 refused · 3 `--tries` re-entries refused · 4 wall · 5 crash · 6 drive gone |
| the control file | `<lane>.control`, read on the minute (1125-1159): `width=N` for the lane's own crew, `<lane>=N` for a hosted crew (N capped at 128), `stop` (acted on once, then the file is emptied; a stale `stop` is cleared at start, 1478-1484). Workers above the new width leave after their document, missing ones are born staggered (1070-1076). `door=URL` or `door=direct` (2026-09-08, HOT-ADD A DOOR) gives the running lane one more crew per role on that door, born the way a launched crew is born - cloud row, outbox landed, a batch claimed, then the ordinary entry with its own exit-pool check, ramp and rate manager - so the VPN walk swaps blocks and rented addresses join without a relaunch; a door already open is not opened twice, a retired door is never re-entered by it, the line is consumed once acted on (1167-1209) |
| the mega lane | `--also LANE:WIDTH` hosts another lane's crew in this process on its own session (778-815); one ramp at a time, `--entry-gap` apart. ONE BATCH (`--one-batch`, 1364-1390, 1295-1322): the hosted crews ride the host's entry - each crew's births start right after the previous crew's ramp, `--stagger` apart, no `--entry-gap`; what closes one crew closes the batch and the batch re-enters from the top, the host first; no rate manager on the batch (1439-1440) |
| the managers | `--manage 1` on a lane alone: the rate manager (the Governor, 156-359) enters with one worker and adds one every `--stagger` s until the band, then adjusts every `--adjust-every` s under `--rps-ceiling` (1039-1040, 1441-1457); the session manager ends the session on purpose at `--session-max-requests` and the cycle re-enters on a fresh batch, no try spent, the wait at the base (1568-1577, 1333-1335). Off by default |
| the heartbeat | `heartbeat()` at the start ("started 1x40", 1513), every minute with the live worker count (1586), with 0 workers and the reason while it waits for the pool, at a hang-up and on the way out (872, 1338, 1602) |

## The fleet

`<Source> Reproduction.py` is a `Site` - the name, the lanes in the cycle's order, their widths, where the lane programs
live, which lanes take `--edge`, the managers' knobs per lane, `one_batch`, `mega_default` (1623-1667) - handed to `main()`
in the fleet part of `rulebook.py` (1615-2110).

| rule | what the fleet does |
|---|---|
| one fleet per machine | `reproduction.lock` in the fleet's folder; its words in `reproduction.log` beside it (1677, 1729) |
| the launch | one process per lane, in the cycle's order, `--entry-gap` (20 s) apart (1766-1775); each lane's `<lane>/<lane>.log` is appended, never truncated, with a fleet banner at every launch (1729-1738). The lane's command line (1695-1727): `--width`, `--host`, `--entry-gap`, `--pending-age` always; `--stagger` / `--redial-wait` / `--tries` only when given, so the lane's own defaults are the cycle's; `--drive` and `--fresh-days` for a documentation crew; `--edge` on a lane's FIRST launch only - afterwards its edge file is the truth (1707-1708); `--limit`, `--unpark`, `--no-pool-check`, `--trust-registry-pages` (documentation); the site's manager knobs for a lane run alone (1724-1726) |
| the mega lane | `--mega`, or a site whose `mega_default` says so (acris): the first lane of `--lanes` hosts the rest as `--also` crews in one process (1676, 1766-1768); on a `one_batch` site the hosted crews get `--one-batch` and no manager knobs (1711-1726); `--separate` launches one process per lane even there (tests) |
| the watch | the children are polled every 3 s, a status line every 60 s, and the fleet is done when every lane has left (1778-1786) |
| what each exit means | 0 stopped cleanly and 1 refused to start: left alone · 2 REFUSED by the source: every lane told to stop, the fleet leaves with 2, a person decides · 3 parked itself (`--tries` re-entries refused) and 4 the wall: never relaunched, a person decides · 5 crash: relaunched · 6 drive gone: relaunched with `--unpark` once the drive is back (1618-1620, 1831-1866, 1868-1889) |
| the relaunch | WAIT_AFTER 60 s (1598; `--relaunch-wait` when not 0, 1864) while the lane's launches in the last hour are within `--relaunch-cap` 3 (1854-1856); past the cap the fleet writes `<lane>.parked` itself and leaves the lane alone (1856-1863); a lane parked meanwhile is not relaunched (1882-1885) |
| `stop` | `stop` into each running lane's control file, up to `--stop-wait` 180 s for the lanes to leave - a lane reads its control file on the minute, then joins its workers - then terminate (1900-1923, 2002-2033); the fleet's own end (a signal, Ctrl+C, a crash) does the same (1788-1798) |
| `status` | this machine's lanes by their locks - RUNNING pid, PARKED with the reason, the control file's word, the log's last line - the fleet's own lock, and every workstation's heartbeat in the cloud within `--within` (1967-1999) |
| `width LANE=N` | `width=N` into the lane's control file, read within a minute; 1 to 128 (2036-2051) |
| `door LANE=URL` | `door=URL` (or `LANE=direct`) into the lane's control file, read within a minute: one more door for the RUNNING lane, its crew born and entered on its own ramp, nothing relaunched (2054-2066; the lane side 1167-1209) |
| a hosted crew | on a mega run a hosted crew takes no lock of its own and reads only its host's control file, so `stop`, `width` and `status` resolve it to its host - the first lane of `--lanes` while that one's lock is alive (`host_of`, 1954-1964): `width registration=20` writes `registration=20` into the host's control file, `stop registration` stops the host's process (every crew leaves), `status` reads the host's lock |

## The board

`<Source> Update.py` hands its source, its lanes and its four flags to `Board` in the board part of `rulebook.py`
(2114-2461); the words are `<Source> Update.md`.

| rule | what the board does |
|---|---|
| the tick | every `--every` 60 s (`--once`: one tick, 2398, 2395): read `machinery.updates` - the totals rows (landed, needed) and every workstation row (workers, last seen, last word, its own landed) - compute, write, remember (2205-2215, 2369-2382). It never counts the workflow table |
| the readings ring | its own memory of the counters, `update.state.json` beside it, KEEP 8 minutes of readings; a restart reads it back (2124, 2136, 2174-2189) |
| one subtraction | rate and increase for 60 s and 5 min come from the reading nearest to that far back and at least three-quarters of the way there (`_then`, 2191-2202): increase = landed now - landed then, rate = increase over the real seconds between, eta = the remaining at that rate (2257-2270); a window with no reading that old stays blank |
| the four statuses | `complete` when needed > 0 and landed >= needed · `stalled` when the lane's last word from its freshest workstation starts with REFUSED or wall (for the phase row: any lane's) · `active` when landed rose in either window · `pending` otherwise (2223-2242, 2271-2278); a workstation row the same from its own word and its own count (2287-2325). eta follows the status: `complete`, `paused` (pending, stalled), or the number (2280-2283) |
| the heartbeats | a workstation is alive when its heartbeat is fresher than `--fresh` 180 s; the lane row's workers is the sum across its workstations alive, its last seen the freshest (2155, 2228-2242). Freshness feeds hosts and width, never the status |
| never clamp | landed outside 0..needed publishes no metrics: the row says OUT OF BOUNDS and names `reconcile` (2250-2253) |
| the write | pct, the two windows, status and as_of = now() onto the row of `machinery.updates`; the totals rows also take the fold - workers, last_seen, last_word (2329-2343) |
| a tick never kills the board | a failed tick logs, keeps the readings, closes the connection and the next tick continues (2409-2414); `update.lock` refuses a second board on the machine (2386). Exit codes: 0 stopped · 5 crash |
| `show` / `reconcile` | `show` reads and prints every row, nothing written (2435-2446); `reconcile` runs `reproduction.reconcile(source)` and prints each counter with its drift, on demand only (2448-2461) |
...
Two checks; only the one-block check is the lane's (`exit_pool` / `wait_for_pool`, rulebook.py:838-875): five draws,
