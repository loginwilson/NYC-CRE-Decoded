...
`build_parser` in `rulebook.py` (2084-2112), from its own help text. The commands: `run` (the default), `status`, `stop [lane]`, `width LANE=N`.
...
The cycle lane is `run(roles, args, here)` in the lane part of `rulebook.py` (658-1626): a lane file defines a ROLE - what one
...
|---|---|
| one door | `<lane>.lock` holds the pid: a second start on this machine is refused while the first lives, a stale lock is taken (716-732, 1491-1492). A parked lane (`<lane>.parked`, the reason inside) refuses to start until `--unpark`, which removes the file (1482-1487). A crew has 1 to MAX_WIDTH 128 workers (847, 1488-1490). The cloud must answer before anything is entered (1525-1530); the outbox left from last time lands first (1531) |
| one entry | one pooled session per crew (`make_session`: a pool of MAX_WIDTH + 4 connections, no retries, 890-901), workers born `--stagger` apart on their own thread, one connection each at its birth, keep-alive after (1035-1066); a crew enters when no other crew ON THE SAME DOOR is ramping (a burst of handshakes on one address is the ban condition; crews with no door share the machine's own line and still take turns) and the last crew on any door started its ramp `--entry-gap` ago (`entered_at`, 938, 1024; the gate 1416-1424) - so DIFFERENT doors, being different addresses, enter `--entry-gap` apart and ramp CONCURRENTLY (2026-09-08: eight doors all ramping within a minute, where the old one-ramp-at-a-time rule took ~5 min per door). The wire must be up: no answer from a neutral host = wait a minute, no try spent (830-841, 1426-1429). `--door`, repeatable, makes one crew per role per door (1511-1515), each crew's session through its own proxy (890-901, 909-912), named `<lane>@doorN` in the log; ONE BATCH and doors together are refused at start |
| the exit-pool check | before every entry, on its own thread so the other crews keep feeding and landing (1430-1438): five draws of the public exit (api.ipify.org, one fresh connection each, 1 s apart, 850-869); five answers in one /24 = enter, else wait 30 s and draw again, the heartbeat saying "waiting for a settled exit pool" (872-887); a verdict older than 120 s is drawn again (1441). `--no-pool-check` skips it. No owner lookup, no spent list: the block is the whole check. Through a door the draws go through the crew's tunnel and answer the rented address, one block by construction (850-869) |
| the batch | the feed keeps the queue a batch ahead: `claim()` for `--claim` (default 12 x the width) with `--ttl`, the registries fetched with the ids (1224-1247); the fresh batch is claimed right before the ramp, never before a wait (1443). An empty list = ask again in 60 s; a failed claim = 30 s (1243, 1214) |
| landing | results go to `<lane>.outbox.jsonl` first and to `land()` second, in chunks of 500, so a cloud hiccup loses nothing (595-656, 1250-1264). The flush: as soon as 200 results wait (1556-1559) and on the minute tick (1581) |
| the minute tick | every 60 s (1573): the control file (1575), the role's check - documentation: the drive is still there, or park with exit 6 (1580) - the landing, the PROGRESS line (1582, 1267-1284), the session manager, the quiet counter, the heartbeat with the live worker count (1583-1603) |
| a worker's failures | a fetch error never stops the lane: the document is written to `<lane>.fails.jsonl` and left for a later pass (963-972); a transport error is re-asked once, then failed, and the cut worker pauses HANGUP_PAUSE_S 5 s before it dials again (998-1008); a `Retry` (a short document, an unknown page shape) is a fail, not a verdict (1009-1013); a worker above the width leaves after its document (1017-1018) |
| the hang-up | the session closed: every worker cut inside HANGUP_WINDOW_S 60 s and nothing landed for HANGUP_QUIET_S 10 s (844-845, 1020-1033; during a ramp the whole width is the workers born). Then at once: leave, land what the crew holds, drop the cut batch (its claims expire on their own), wait `--redial-wait` with no line open, re-enter once on a fresh batch (1337-1366) |
| served or refused | the last re-entry was SERVED when it landed SERVED_LANDINGS 300 or lived SERVED_S 300 s (846, 1322): the incident closes and the wait halves back toward the base (1349-1350); otherwise it was REFUSED and the next wait doubles, cap 4,800 s (1352); `--tries` refused in a row = park, exit 3 (1353-1354). The try is spent at the re-entry itself, never while the wire is down (1444-1447) |
| dead transport | 3 x width transport errors in a row and nothing landed for 60 s = the same hang-up (1567-1569); five minute ticks asking with nothing landing = "our wire", the same hang-up (1593-1598) |
| the wall | 40 consecutive 503/429 (the wall counts these two only) = park, exit 4, not retrying, not rotating (995-996, 1561-1563) |
| the refusal | the source's notice page (`Refused`, raised by the source module's detector) = park at once, exit 2 (990-991). With more than one `--door`, the notice on one door retires that door's crew alone - its lines close, what it fetched lands, its claims expire (1102-1111, 1542-1551) - and the lane parks when the last open door is refused |
| the park | `<lane>.parked` with the reason; the process stops; the heartbeat carries the reason on the way out (1113-1124, 1612-1625). Exit codes: 0 stopped (the control file, `--limit`, Ctrl+C, a signal) · 2 refused · 3 `--tries` re-entries refused · 4 wall · 5 crash · 6 drive gone |
| the control file | `<lane>.control`, read on the minute (1137-1171): `width=N` for the lane's own crew, `<lane>=N` for a hosted crew (N capped at 128), `stop` (acted on once, then the file is emptied; a stale `stop` is cleared at start, 1493-1499). Workers above the new width leave after their document, missing ones are born staggered (1082-1088). `door=URL` or `door=direct` (2026-09-08, HOT-ADD A DOOR) gives the running lane one more crew per role on that door, born the way a launched crew is born - cloud row, outbox landed, a batch claimed, then the ordinary entry with its own exit-pool check, ramp and rate manager - so the VPN walk swaps blocks and rented addresses join without a relaunch; a door already open is not opened twice, a retired door is never re-entered by it, the line is consumed once acted on (1179-1221) |
| the mega lane | `--also LANE:WIDTH` hosts another lane's crew in this process on its own session (790-827); one ramp at a time, `--entry-gap` apart. ONE BATCH (`--one-batch`, 1376-1402, 1307-1334): the hosted crews ride the host's entry - each crew's births start right after the previous crew's ramp, `--stagger` apart, no `--entry-gap`; what closes one crew closes the batch and the batch re-enters from the top, the host first; no rate manager on the batch (1451-1452) |
| the managers | `--manage 1` on a lane alone: the rate manager (the Governor, 156-359) enters with one worker and adds one every `--stagger` s until the band, then adjusts every `--adjust-every` s under `--rps-ceiling` (1051-1052, 1453-1469); the session manager ends the session on purpose at `--session-max-requests` and the cycle re-enters on a fresh batch, no try spent, the wait at the base (1583-1592, 1345-1347). Off by default |
| the heartbeat | `heartbeat()` at the start ("started 1x40", 1528), every minute with the live worker count (1601), with 0 workers and the reason while it waits for the pool, at a hang-up and on the way out (884, 1338, 1617) |

## The fleet

`<Source> Reproduction.py` is a `Site` - the name, the lanes in the cycle's order, their widths, where the lane programs
live, which lanes take `--edge`, the managers' knobs per lane, `one_batch`, `mega_default` (1638-1682) - handed to `main()`
in the fleet part of `rulebook.py` (1630-2125).

| rule | what the fleet does |
|---|---|
| one fleet per machine | `reproduction.lock` in the fleet's folder; its words in `reproduction.log` beside it (1692, 1729) |
| the launch | one process per lane, in the cycle's order, `--entry-gap` (20 s) apart (1781-1790); each lane's `<lane>/<lane>.log` is appended, never truncated, with a fleet banner at every launch (1744-1753). The lane's command line (1710-1742): `--width`, `--host`, `--entry-gap`, `--pending-age` always; `--stagger` / `--redial-wait` / `--tries` only when given, so the lane's own defaults are the cycle's; `--drive` and `--fresh-days` for a documentation crew; `--edge` on a lane's FIRST launch only - afterwards its edge file is the truth (1722-1723); `--limit`, `--unpark`, `--no-pool-check`, `--trust-registry-pages` (documentation); the site's manager knobs for a lane run alone (1739-1741) |
| the mega lane | `--mega`, or a site whose `mega_default` says so (acris): the first lane of `--lanes` hosts the rest as `--also` crews in one process (1691, 1781-1783); on a `one_batch` site the hosted crews get `--one-batch` and no manager knobs (1726-1741); `--separate` launches one process per lane even there (tests) |
| the watch | the children are polled every 3 s, a status line every 60 s, and the fleet is done when every lane has left (1793-1801) |
| what each exit means | 0 stopped cleanly and 1 refused to start: left alone · 2 REFUSED by the source: every lane told to stop, the fleet leaves with 2, a person decides · 3 parked itself (`--tries` re-entries refused) and 4 the wall: never relaunched, a person decides · 5 crash: relaunched · 6 drive gone: relaunched with `--unpark` once the drive is back (1633-1635, 1846-1881, 1883-1904) |
| the relaunch | WAIT_AFTER 60 s (1598; `--relaunch-wait` when not 0, 1879) while the lane's launches in the last hour are within `--relaunch-cap` 3 (1869-1871); past the cap the fleet writes `<lane>.parked` itself and leaves the lane alone (1871-1878); a lane parked meanwhile is not relaunched (1897-1900) |
| `stop` | `stop` into each running lane's control file, up to `--stop-wait` 180 s for the lanes to leave - a lane reads its control file on the minute, then joins its workers - then terminate (1915-1938, 2017-2048); the fleet's own end (a signal, Ctrl+C, a crash) does the same (1803-1813) |
| `status` | this machine's lanes by their locks - RUNNING pid, PARKED with the reason, the control file's word, the log's last line - the fleet's own lock, and every workstation's heartbeat in the cloud within `--within` (1982-2014) |
| `width LANE=N` | `width=N` into the lane's control file, read within a minute; 1 to 128 (2051-2066) |
| `door LANE=URL` | `door=URL` (or `LANE=direct`) into the lane's control file, read within a minute: one more door for the RUNNING lane, its crew born and entered on its own ramp, nothing relaunched (2069-2081; the lane side 1179-1221) |
| a hosted crew | on a mega run a hosted crew takes no lock of its own and reads only its host's control file, so `stop`, `width` and `status` resolve it to its host - the first lane of `--lanes` while that one's lock is alive (`host_of`, 1969-1979): `width registration=20` writes `registration=20` into the host's control file, `stop registration` stops the host's process (every crew leaves), `status` reads the host's lock |

## The board

`<Source> Update.py` hands its source, its lanes and its four flags to `Board` in the board part of `rulebook.py`
(2129-2476); the words are `<Source> Update.md`.

| rule | what the board does |
|---|---|
| the tick | every `--every` 60 s (`--once`: one tick, 2413, 2395): read `machinery.updates` - the totals rows (landed, needed) and every workstation row (workers, last seen, last word, its own landed) - compute, write, remember (2220-2230, 2384-2397). It never counts the workflow table |
| the readings ring | its own memory of the counters, `update.state.json` beside it, KEEP 8 minutes of readings; a restart reads it back (2139, 2136, 2189-2204) |
| one subtraction | rate and increase for 60 s and 5 min come from the reading nearest to that far back and at least three-quarters of the way there (`_then`, 2206-2217): increase = landed now - landed then, rate = increase over the real seconds between, eta = the remaining at that rate (2272-2285); a window with no reading that old stays blank |
| the four statuses | `complete` when needed > 0 and landed >= needed · `stalled` when the lane's last word from its freshest workstation starts with REFUSED or wall (for the phase row: any lane's) · `active` when landed rose in either window · `pending` otherwise (2238-2257, 2286-2293); a workstation row the same from its own word and its own count (2302-2340). eta follows the status: `complete`, `paused` (pending, stalled), or the number (2295-2298) |
| the heartbeats | a workstation is alive when its heartbeat is fresher than `--fresh` 180 s; the lane row's workers is the sum across its workstations alive, its last seen the freshest (2170, 2243-2257). Freshness feeds hosts and width, never the status |
| never clamp | landed outside 0..needed publishes no metrics: the row says OUT OF BOUNDS and names `reconcile` (2265-2268) |
| the write | pct, the two windows, status and as_of = now() onto the row of `machinery.updates`; the totals rows also take the fold - workers, last_seen, last_word (2344-2358) |
| a tick never kills the board | a failed tick logs, keeps the readings, closes the connection and the next tick continues (2424-2429); `update.lock` refuses a second board on the machine (2401). Exit codes: 0 stopped · 5 crash |
| `show` / `reconcile` | `show` reads and prints every row, nothing written (2450-2461); `reconcile` runs `reproduction.reconcile(source)` and prints each counter with its drift, on demand only (2463-2476) |
