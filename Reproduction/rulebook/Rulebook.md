...
`build_parser` in `rulebook.py` (1977-2005), from its own help text. The commands: `run` (the default), `status`, `stop [lane]`, `width LANE=N`.
...
The cycle lane is `run(roles, args, here)` in the lane part of `rulebook.py` (649-1534): a lane file defines a ROLE - what one
...
|---|---|
| one door | `<lane>.lock` holds the pid: a second start on this machine is refused while the first lives, a stale lock is taken (698-714, 1399-1400). A parked lane (`<lane>.parked`, the reason inside) refuses to start until `--unpark`, which removes the file (1390-1395). A crew has 1 to MAX_WIDTH 128 workers (823, 1396-1398). The cloud must answer before anything is entered (1433-1438); the outbox left from last time lands first (1439) |
| one entry | one pooled session per crew (`make_session`: a pool of MAX_WIDTH + 4 connections, no retries, 866-877), workers born `--stagger` apart on their own thread, one connection each at its birth, keep-alive after (1009-1039); a crew enters only when no other crew is ramping and the last ramp ended `--entry-gap` ago (1333-1335). The wire must be up: no answer from a neutral host = wait a minute, no try spent (806-817, 1337-1340). `--door`, repeatable, makes one crew per role per door (1419-1423), each crew's session through its own proxy (866-877, 885-887), named `<lane>@doorN` in the log; ONE BATCH and doors together are refused at start |
| the exit-pool check | before every entry, on its own thread so the other crews keep feeding and landing (1341-1349): five draws of the public exit (api.ipify.org, one fresh connection each, 1 s apart, 826-845); five answers in one /24 = enter, else wait 30 s and draw again, the heartbeat saying "waiting for a settled exit pool" (848-863); a verdict older than 120 s is drawn again (1352). `--no-pool-check` skips it. No owner lookup, no spent list: the block is the whole check. Through a door the draws go through the crew's tunnel and answer the rented address, one block by construction (826-845) |
| the batch | the feed keeps the queue a batch ahead: `claim()` for `--claim` (default 12 x the width) with `--ttl`, the registries fetched with the ids (1141-1164); the fresh batch is claimed right before the ramp, never before a wait (1354). An empty list = ask again in 60 s; a failed claim = 30 s (1160, 1157) |
| landing | results go to `<lane>.outbox.jsonl` first and to `land()` second, in chunks of 500, so a cloud hiccup loses nothing (586-647, 1167-1181). The flush: as soon as 200 results wait (1464-1467) and on the minute tick (1489) |
| the minute tick | every 60 s (1481): the control file (1483), the role's check - documentation: the drive is still there, or park with exit 6 (1488) - the landing, the PROGRESS line (1490, 1184-1201), the session manager, the quiet counter, the heartbeat with the live worker count (1491-1511) |
| a worker's failures | a fetch error never stops the lane: the document is written to `<lane>.fails.jsonl` and left for a later pass (937-946); a transport error is re-asked once, then failed, and the cut worker pauses HANGUP_PAUSE_S 5 s before it dials again (972-982); a `Retry` (a short document, an unknown page shape) is a fail, not a verdict (983-987); a worker above the width leaves after its document (991-992) |
| the hang-up | the session closed: every worker cut inside HANGUP_WINDOW_S 60 s and nothing landed for HANGUP_QUIET_S 10 s (820-821, 994-1007; during a ramp the whole width is the workers born). Then at once: leave, land what the crew holds, drop the cut batch (its claims expire on their own), wait `--redial-wait` with no line open, re-enter once on a fresh batch (1254-1283) |
| served or refused | the last re-entry was SERVED when it landed SERVED_LANDINGS 300 or lived SERVED_S 300 s (822, 1265): the incident closes and the wait halves back toward the base (1266-1267); otherwise it was REFUSED and the next wait doubles, cap 4,800 s (1269); `--tries` refused in a row = park, exit 3 (1270-1271). The try is spent at the re-entry itself, never while the wire is down (1355-1358) |
| dead transport | 3 x width transport errors in a row and nothing landed for 60 s = the same hang-up (1475-1477); five minute ticks asking with nothing landing = "our wire", the same hang-up (1501-1506) |
| the wall | 40 consecutive 503/429 (the wall counts these two only) = park, exit 4, not retrying, not rotating (969-970, 1469-1471) |
| the refusal | the source's notice page (`Refused`, raised by the source module's detector) = park at once, exit 2 (964-965). With more than one `--door`, the notice on one door retires that door's crew alone - its lines close, what it fetched lands, its claims expire (1075-1084, 1450-1459) - and the lane parks when the last open door is refused |
| the park | `<lane>.parked` with the reason; the process stops; the heartbeat carries the reason on the way out (1086-1097, 1520-1533). Exit codes: 0 stopped (the control file, `--limit`, Ctrl+C, a signal) · 2 refused · 3 `--tries` re-entries refused · 4 wall · 5 crash · 6 drive gone |
| the control file | `<lane>.control`, read on the minute (1110-1138): `width=N` for the lane's own crew, `<lane>=N` for a hosted crew (N capped at 128), `stop` (acted on once, then the file is emptied; a stale `stop` is cleared at start, 1401-1407). Workers above the new width leave after their document, missing ones are born staggered (1055-1061) |
| the mega lane | `--also LANE:WIDTH` hosts another lane's crew in this process on its own session (766-803); one ramp at a time, `--entry-gap` apart. ONE BATCH (`--one-batch`, 1293-1319, 1224-1251): the hosted crews ride the host's entry - each crew's births start right after the previous crew's ramp, `--stagger` apart, no `--entry-gap`; what closes one crew closes the batch and the batch re-enters from the top, the host first; no rate manager on the batch (1362-1363) |
| the managers | `--manage 1` on a lane alone: the rate manager (the Governor, 156-359) enters with one worker and adds one every `--stagger` s until the band, then adjusts every `--adjust-every` s under `--rps-ceiling` (1024-1025, 1364-1380); the session manager ends the session on purpose at `--session-max-requests` and the cycle re-enters on a fresh batch, no try spent, the wait at the base (1491-1500, 1262-1264). Off by default |
| the heartbeat | `heartbeat()` at the start ("started 1x40", 1436), every minute with the live worker count (1509), with 0 workers and the reason while it waits for the pool, at a hang-up and on the way out (860, 1281, 1525) |

## The fleet

`<Source> Reproduction.py` is a `Site` - the name, the lanes in the cycle's order, their widths, where the lane programs
live, which lanes take `--edge`, the managers' knobs per lane, `one_batch`, `mega_default` (1546-1590) - handed to `main()`
in the fleet part of `rulebook.py` (1538-2016).

| rule | what the fleet does |
|---|---|
| one fleet per machine | `reproduction.lock` in the fleet's folder; its words in `reproduction.log` beside it (1600, 1672) |
| the launch | one process per lane, in the cycle's order, `--entry-gap` (20 s) apart (1689-1698); each lane's `<lane>/<lane>.log` is appended, never truncated, with a fleet banner at every launch (1652-1661). The lane's command line (1618-1650): `--width`, `--host`, `--entry-gap`, `--pending-age` always; `--stagger` / `--redial-wait` / `--tries` only when given, so the lane's own defaults are the cycle's; `--drive` and `--fresh-days` for a documentation crew; `--edge` on a lane's FIRST launch only - afterwards its edge file is the truth (1630-1631); `--limit`, `--unpark`, `--no-pool-check`, `--trust-registry-pages` (documentation); the site's manager knobs for a lane run alone (1647-1649) |
| the mega lane | `--mega`, or a site whose `mega_default` says so (acris): the first lane of `--lanes` hosts the rest as `--also` crews in one process (1599, 1689-1691); on a `one_batch` site the hosted crews get `--one-batch` and no manager knobs (1634-1649); `--separate` launches one process per lane even there (tests) |
| the watch | the children are polled every 3 s, a status line every 60 s, and the fleet is done when every lane has left (1701-1709) |
| what each exit means | 0 stopped cleanly and 1 refused to start: left alone · 2 REFUSED by the source: every lane told to stop, the fleet leaves with 2, a person decides · 3 parked itself (`--tries` re-entries refused) and 4 the wall: never relaunched, a person decides · 5 crash: relaunched · 6 drive gone: relaunched with `--unpark` once the drive is back (1541-1543, 1754-1789, 1791-1812) |
| the relaunch | WAIT_AFTER 60 s (1541; `--relaunch-wait` when not 0, 1787) while the lane's launches in the last hour are within `--relaunch-cap` 3 (1777-1779); past the cap the fleet writes `<lane>.parked` itself and leaves the lane alone (1779-1786); a lane parked meanwhile is not relaunched (1805-1808) |
| `stop` | `stop` into each running lane's control file, up to `--stop-wait` 180 s for the lanes to leave - a lane reads its control file on the minute, then joins its workers - then terminate (1823-1846, 1925-1956); the fleet's own end (a signal, Ctrl+C, a crash) does the same (1711-1721) |
| `status` | this machine's lanes by their locks - RUNNING pid, PARKED with the reason, the control file's word, the log's last line - the fleet's own lock, and every workstation's heartbeat in the cloud within `--within` (1890-1922) |
| `width LANE=N` | `width=N` into the lane's control file, read within a minute; 1 to 128 (1959-1974) |
| a hosted crew | on a mega run a hosted crew takes no lock of its own and reads only its host's control file, so `stop`, `width` and `status` resolve it to its host - the first lane of `--lanes` while that one's lock is alive (`host_of`, 1877-1887): `width registration=20` writes `registration=20` into the host's control file, `stop registration` stops the host's process (every crew leaves), `status` reads the host's lock |

## The board

`<Source> Update.py` hands its source, its lanes and its four flags to `Board` in the board part of `rulebook.py`
(2020-2367); the words are `<Source> Update.md`.

| rule | what the board does |
|---|---|
| the tick | every `--every` 60 s (`--once`: one tick, 2304, 2321): read `machinery.updates` - the totals rows (landed, needed) and every workstation row (workers, last seen, last word, its own landed) - compute, write, remember (2111-2121, 2275-2288). It never counts the workflow table |
| the readings ring | its own memory of the counters, `update.state.json` beside it, KEEP 8 minutes of readings; a restart reads it back (2030, 2062, 2080-2095) |
| one subtraction | rate and increase for 60 s and 5 min come from the reading nearest to that far back and at least three-quarters of the way there (`_then`, 2097-2108): increase = landed now - landed then, rate = increase over the real seconds between, eta = the remaining at that rate (2163-2176); a window with no reading that old stays blank |
| the four statuses | `complete` when needed > 0 and landed >= needed · `stalled` when the lane's last word from its freshest workstation starts with REFUSED or wall (for the phase row: any lane's) · `active` when landed rose in either window · `pending` otherwise (2129-2148, 2177-2184); a workstation row the same from its own word and its own count (2193-2231). eta follows the status: `complete`, `paused` (pending, stalled), or the number (2186-2189) |
| the heartbeats | a workstation is alive when its heartbeat is fresher than `--fresh` 180 s; the lane row's workers is the sum across its workstations alive, its last seen the freshest (2061, 2134-2148). Freshness feeds hosts and width, never the status |
| never clamp | landed outside 0..needed publishes no metrics: the row says OUT OF BOUNDS and names `reconcile` (2156-2159) |
| the write | pct, the two windows, status and as_of = now() onto the row of `machinery.updates`; the totals rows also take the fold - workers, last_seen, last_word (2235-2249) |
| a tick never kills the board | a failed tick logs, keeps the readings, closes the connection and the next tick continues (2315-2320); `update.lock` refuses a second board on the machine (2292). Exit codes: 0 stopped · 5 crash |
| `show` / `reconcile` | `show` reads and prints every row, nothing written (2341-2352); `reconcile` runs `reproduction.reconcile(source)` and prints each counter with its drift, on demand only (2354-2367) |
