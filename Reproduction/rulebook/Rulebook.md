...
Each was a numbered file, applied once; since 0016 (2026-09-07) they are folded into the one file `supabase/schema.sql` and the ledger keeps the versions - nineteen applied. One line per version, from `supabase/Supabase.md`'s log:
...
`build_parser` in `rulebook.py` (1931-1956), from its own help text. The commands: `run` (the default), `status`, `stop [lane]`, `width LANE=N`.
...
The cycle lane is `run(roles, args, here)` in the lane part of `rulebook.py` (650-1490): a lane file defines a ROLE - what one
...
|---|---|
| one door | `<lane>.lock` holds the pid: a second start on this machine is refused while the first lives, a stale lock is taken (699-715, 1373-1374). A parked lane (`<lane>.parked`, the reason inside) refuses to start until `--unpark`, which removes the file (1364-1369). A crew has 1 to MAX_WIDTH 128 workers (819, 1370-1372). The cloud must answer before anything is entered (1401-1406); the outbox left from last time lands first (1407) |
| one entry | one pooled session per crew (`make_session`: a pool of MAX_WIDTH + 4 connections, no retries, 858-866), workers born `--stagger` apart on their own thread, one connection each at its birth, keep-alive after (995-1025); a crew enters only when no other crew is ramping and the last ramp ended `--entry-gap` ago (1307-1309). The wire must be up: no answer from a neutral host = wait a minute, no try spent (802-813, 1311-1314) |
| the exit-pool check | before every entry, on its own thread so the other crews keep feeding and landing (1315-1323): five draws of the public exit (api.ipify.org, one fresh connection each, 1 s apart, 822-837); five answers in one /24 = enter, else wait 30 s and draw again, the heartbeat saying "waiting for a settled exit pool" (840-855); a verdict older than 120 s is drawn again (1326). `--no-pool-check` skips it. No owner lookup, no spent list: the block is the whole check |
| the batch | the feed keeps the queue a batch ahead: `claim()` for `--claim` (default 12 x the width) with `--ttl`, the registries fetched with the ids (1115-1138); the fresh batch is claimed right before the ramp, never before a wait (1328). An empty list = ask again in 60 s; a failed claim = 30 s (1134, 1131) |
| landing | results go to `<lane>.outbox.jsonl` first and to `land()` second, in chunks of 500, so a cloud hiccup loses nothing (587-648, 1141-1155). The flush: as soon as 200 results wait (1422-1425) and on the minute tick (1445) |
| the minute tick | every 60 s (1439): the control file (1441), the role's check - documentation: the drive is still there, or park with exit 6 (1444) - the landing, the PROGRESS line (1446, 1158-1175), the session manager, the quiet counter, the heartbeat with the live worker count (1447-1467) |
| a worker's failures | a fetch error never stops the lane: the document is written to `<lane>.fails.jsonl` and left for a later pass (923-932); a transport error is re-asked once, then failed, and the cut worker pauses HANGUP_PAUSE_S 5 s before it dials again (958-968); a `Retry` (a short document, an unknown page shape) is a fail, not a verdict (969-973); a worker above the width leaves after its document (977-978) |
| the hang-up | the session closed: every worker cut inside HANGUP_WINDOW_S 60 s and nothing landed for HANGUP_QUIET_S 10 s (816-817, 980-993; during a ramp the whole width is the workers born). Then at once: leave, land what the crew holds, drop the cut batch (its claims expire on their own), wait `--redial-wait` with no line open, re-enter once on a fresh batch (1228-1257) |
| served or refused | the last re-entry was SERVED when it landed SERVED_LANDINGS 300 or lived SERVED_S 300 s (818, 1239): the incident closes and the wait halves back toward the base (1240-1241); otherwise it was REFUSED and the next wait doubles, cap 4,800 s (1243); `--tries` refused in a row = park, exit 3 (1244-1245). The try is spent at the re-entry itself, never while the wire is down (1329-1332) |
| dead transport | 3 x width transport errors in a row and nothing landed for 60 s = the same hang-up (1433-1435); five minute ticks asking with nothing landing = "our wire", the same hang-up (1457-1462) |
| the wall | 40 consecutive 503/429 (the wall counts these two only) = park, exit 4, not retrying, not rotating (955-956, 1427-1429) |
| the refusal | the source's notice page (`Refused`, raised by the source module's detector) = park at once, exit 2 (950-951) |
| the park | `<lane>.parked` with the reason; the process stops; the heartbeat carries the reason on the way out (1060-1071, 1476-1489). Exit codes: 0 stopped (the control file, `--limit`, Ctrl+C, a signal) · 2 refused · 3 `--tries` re-entries refused · 4 wall · 5 crash · 6 drive gone |
| the control file | `<lane>.control`, read on the minute (1084-1112): `width=N` for the lane's own crew, `<lane>=N` for a hosted crew (N capped at 128), `stop` (acted on once, then the file is emptied; a stale `stop` is cleared at start, 1375-1381). Workers above the new width leave after their document, missing ones are born staggered (1041-1047) |
| the mega lane | `--also LANE:WIDTH` hosts another lane's crew in this process on its own session (762-799); one ramp at a time, `--entry-gap` apart. ONE BATCH (`--one-batch`, 1267-1293, 1198-1225): the hosted crews ride the host's entry - each crew's births start right after the previous crew's ramp, `--stagger` apart, no `--entry-gap`; what closes one crew closes the batch and the batch re-enters from the top, the host first; no rate manager on the batch (1336-1337) |
| the managers | `--manage 1` on a lane alone: the rate manager (the Governor, 157-360) enters with one worker and adds one every `--stagger` s until the band, then adjusts every `--adjust-every` s under `--rps-ceiling` (1010-1011, 1338-1354); the session manager ends the session on purpose at `--session-max-requests` and the cycle re-enters on a fresh batch, no try spent, the wait at the base (1447-1456, 1236-1238). Off by default |
| the heartbeat | `heartbeat()` at the start ("started 1x40", 1404), every minute with the live worker count (1465), with 0 workers and the reason while it waits for the pool, at a hang-up and on the way out (852, 1255, 1481) |

## The fleet

`<Source> Reproduction.py` is a `Site` - the name, the lanes in the cycle's order, their widths, where the lane programs
live, which lanes take `--edge`, the managers' knobs per lane, `one_batch`, `mega_default` (1502-1546) - handed to `main()`
in the fleet part of `rulebook.py` (1493-1967).

| rule | what the fleet does |
|---|---|
| one fleet per machine | `reproduction.lock` in the fleet's folder; its words in `reproduction.log` beside it (1556, 1626) |
| the launch | one process per lane, in the cycle's order, `--entry-gap` (20 s) apart (1643-1652); each lane's `<lane>/<lane>.log` is appended, never truncated, with a fleet banner at every launch (1606-1615). The lane's command line (1574-1604): `--width`, `--host`, `--entry-gap`, `--pending-age` always; `--stagger` / `--redial-wait` / `--tries` only when given, so the lane's own defaults are the cycle's; `--drive` and `--fresh-days` for a documentation crew; `--edge` on a lane's FIRST launch only - afterwards its edge file is the truth (1586-1587); `--limit`, `--unpark`, `--no-pool-check`, `--trust-registry-pages` (documentation); the site's manager knobs for a lane run alone (1601-1603) |
| the mega lane | `--mega`, or a site whose `mega_default` says so (acris): the first lane of `--lanes` hosts the rest as `--also` crews in one process (1555, 1643-1645); on a `one_batch` site the hosted crews get `--one-batch` and no manager knobs (1590-1603); `--separate` launches one process per lane even there (tests) |
| the watch | the children are polled every 3 s, a status line every 60 s, and the fleet is done when every lane has left (1655-1663) |
| what each exit means | 0 stopped cleanly and 1 refused to start: left alone · 2 REFUSED by the source: every lane told to stop, the fleet leaves with 2, a person decides · 3 parked itself (`--tries` re-entries refused) and 4 the wall: never relaunched, a person decides · 5 crash: relaunched · 6 drive gone: relaunched with `--unpark` once the drive is back (1497-1499, 1708-1743, 1745-1766) |
| the relaunch | WAIT_AFTER 60 s (1497; `--relaunch-wait` when not 0, 1741) while the lane's launches in the last hour are within `--relaunch-cap` 3 (1731-1733); past the cap the fleet writes `<lane>.parked` itself and leaves the lane alone (1733-1740); a lane parked meanwhile is not relaunched (1759-1762) |
| `stop` | `stop` into each running lane's control file, up to `--stop-wait` 180 s for the lanes to leave - a lane reads its control file on the minute, then joins its workers - then terminate (1777-1800, 1879-1910); the fleet's own end (a signal, Ctrl+C, a crash) does the same (1665-1675) |
| `status` | this machine's lanes by their locks - RUNNING pid, PARKED with the reason, the control file's word, the log's last line - the fleet's own lock, and every workstation's heartbeat in the cloud within `--within` (1844-1876) |
| `width LANE=N` | `width=N` into the lane's control file, read within a minute; 1 to 128 (1913-1928) |
| a hosted crew | on a mega run a hosted crew takes no lock of its own and reads only its host's control file, so `stop`, `width` and `status` resolve it to its host - the first lane of `--lanes` while that one's lock is alive (`host_of`, 1831-1841): `width registration=20` writes `registration=20` into the host's control file, `stop registration` stops the host's process (every crew leaves), `status` reads the host's lock |

## The board

`<Source> Update.py` hands its source, its lanes and its four flags to `Board` in the board part of `rulebook.py`
(1970-2318); the words are `<Source> Update.md`.

| rule | what the board does |
|---|---|
| the tick | every `--every` 60 s (`--once`: one tick, 2255, 2272): read `machinery.updates` - the totals rows (landed, needed) and every workstation row (workers, last seen, last word, its own landed) - compute, write, remember (2062-2072, 2226-2239). It never counts the workflow table |
| the readings ring | its own memory of the counters, `update.state.json` beside it, KEEP 8 minutes of readings; a restart reads it back (1981, 2013, 2031-2046) |
| one subtraction | rate and increase for 60 s and 5 min come from the reading nearest to that far back and at least three-quarters of the way there (`_then`, 2048-2059): increase = landed now - landed then, rate = increase over the real seconds between, eta = the remaining at that rate (2114-2127); a window with no reading that old stays blank |
| the four statuses | `complete` when needed > 0 and landed >= needed · `stalled` when the lane's last word from its freshest workstation starts with REFUSED or wall (for the phase row: any lane's) · `active` when landed rose in either window · `pending` otherwise (2080-2099, 2128-2135); a workstation row the same from its own word and its own count (2144-2182). eta follows the status: `complete`, `paused` (pending, stalled), or the number (2137-2140) |
| the heartbeats | a workstation is alive when its heartbeat is fresher than `--fresh` 180 s; the lane row's workers is the sum across its workstations alive, its last seen the freshest (2012, 2085-2099). Freshness feeds hosts and width, never the status |
| never clamp | landed outside 0..needed publishes no metrics: the row says OUT OF BOUNDS and names `reconcile` (2107-2110) |
| the write | pct, the two windows, status and as_of = now() onto the row of `machinery.updates`; the totals rows also take the fold - workers, last_seen, last_word (2186-2200) |
| a tick never kills the board | a failed tick logs, keeps the readings, closes the connection and the next tick continues (2266-2271); `update.lock` refuses a second board on the machine (2243). Exit codes: 0 stopped · 5 crash |
| `show` / `reconcile` | `show` reads and prints every row, nothing written (2292-2303); `reconcile` runs `reproduction.reconcile(source)` and prints each counter with its drift, on demand only (2305-2318) |
...
Two checks; only the one-block check is the lane's (`exit_pool` / `wait_for_pool`, rulebook.py:822-855): five draws,
