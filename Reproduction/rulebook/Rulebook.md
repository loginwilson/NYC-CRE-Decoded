...
| `cloud.py` | THE CLOUD TABLE from a lane's point of view: `claim`, `land`, `heartbeat` as one round trip each to the functions in the migrations, the registries, and the local outbox so a cloud hiccup loses nothing. One connection per crew, one statement at a time under its lock | `test_lane_sim.py`, `test_lane_policies.py` (throwaway rows on the live table) |
