...
- **2026-09-07 00:11 — the fourteenth notice; the A/B is closed.** The proven OneTouch lane (1x40 pinned, no manager) served 6.8 min / 26,279 requests / 2,422 pdfs on a fresh VPN block, then the notice. Three fresh blocks tonight fell at 22-26k requests each at three different rates: the meter tonight is a per-exit REQUEST BUDGET on this provider's ranges, not a rate and not the code (old and new meet the same wall). Gate 4 waits on an address, not a change: workstation 2's own line or another provider. See Acris Reproduction.md.
