...
**The status follows the lane.** Each lane writes its own heartbeat into its update row (host, width, a timestamp every minute, its last word), so the board can say: `active` = heartbeat fresh and landed rising · `pending` = no fresh heartbeat and not complete (the lane is paused or parked by a person) · `stalled` = the lane's last word is a refusal or a wall (rejected by the source) · `complete` = 100 %. A fetch error never stops a lane; only the notice page does, and a hang-up is redialed.
...
| `20260903230000_pending_backfill.sql` | pendings first: `claim()` takes a sixth argument `pending_age` and offers the pendings due for a re-check before the empties; the pending indexes are re-keyed on `updated_at` so the due set is an index range (login 2026-09-03 23:5x, "pending goes back to the backfill") |
