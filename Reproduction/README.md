...
Six files every lane shares, so a rule is fixed once and every lane on every workstation gets it: `fleet.py` (the fleet both reproduction programs run on), `board.py` (the board both update programs run on), `lane.py` (the entry: one pooled session per crew, staggered births, failures, retries, the refusal park, the hang-up redial, the wall stop, live width changes, the mega lane, claim/land/heartbeat once a minute), `cloud.py` (the cloud table from a lane's point of view: claim, registries, land, heartbeat, and the local outbox so a cloud hiccup loses nothing), `storage.py` (the drive found by its label on Windows or Mac, and the One Touch layout `source\borough\year\month\id.pdf` recorded in canonical `D:\` form whichever machine fetched the file). `requirements.txt` is the one install a workstation needs. `rate_manager.py` is the sixth: the rate and session managers a lane runs with `--manage 1` (the acris fleet passes it to documentation), proven offline by `test_managers.py` beside it - `python test_managers.py`, nothing asked of any source.
