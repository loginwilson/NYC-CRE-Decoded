...
Any workstation can run the diff and the census (they read the index and the table). The probe asks ACRIS itself and runs on login's word only: `--acris` is that word, and the program refuses to start while any lane's heartbeat is fresh, because an enumeration sweep of the web endpoint never runs beside the cycle. Exit codes: 0 the difference is 0 · 1 a difference · 7 unproven · 2 refused · 3 hang-up · 5 crash. A refusal to start (`--probe` without `--acris`, a fresh lane heartbeat, a parked probe) leaves with 5 like a crash.

This program builds its own command line and takes none of the lanes' shared flags: `--all` · `--months` (without `--all`, the newest N months of the index, 3) · `--shard` (one shard: a month `YYYYMM`, `FT_<borough><digit>` or `BK_<yy>`; repeatable) · `--census` · `--probe` · `--acris` (login's word) · `--years` (census or probe: `2016`, `2016,2024`, `2003-2010`; default every CRFN year) · `--retop` (probe: gallop each year's top again even if the journal holds one) · `--width` (the probe's connections, 10) · `--stagger` (seconds between births, 5) · `--redial-wait` (60; ×2 refused, ÷2 served) · `--tries` (4) · `--unpark` · `--host` (this workstation's name; default the machine name).
...
| probe width | 10 connections, births 5 s apart | the void walk's number (2026-08-21: 10 workers, one session each, no refusal); the ramp is the entry the door serves (2026-09-04); the probe is a small run |
| asks per number | 3 | then UNKNOWN; the gallop's numbers likewise |
| hang-up | every connection inside 60 s with nothing answered for 10 s; then 60 s of silence with the backoff, four re-entries | the cycle's rule (Acris Documentation.md) |
...
2026-09-07 — the audit of the source folder against the code: the program's own flags named under Launch (it never calls `add_common_args`, so the shared-flags sentence was false - `--drive`, `--claim`, `--limit` and the rest are refused as unknown), `enumeration.probe.tmp` under Working files and the exit-5 sentence beside the exit codes (they had sat under this history); `transport_streak` - written, never read - left the program; this history in one order, oldest first.
