...
| trust-registry-pages | off | skip the viewer fetch; `total` = the registry's `pages` (positive only; otherwise the viewer decides). Proposed, untested against the source: A/B first |
...
`Reproduction/rulebook/rulebook.py` (the entry and the policies every lane shares; claim, land, heartbeat, the outbox), `Reproduction/rulebook/rulebook.py` (the drive by label, the One Touch layout), `Reproduction/Acris/rulebook/acris.py` (the ACRIS rules: URLs minted from the id, the one user-agent, the refusal detector, the page count, where a document files).
...
2026-09-07 02:1x — `--trust-registry-pages` (PROPOSED, off by default; login's timer: "we need to be able to scale to a reasonable pace"). The calendar is total requests / (57 req/s x exits); the only code lever that moves it is fewer requests per document. `fetch()` spends one request per document on the viewer page just to read `TotalPages` - but registration already recorded `pages` for every row (the cut document 2005081702043001 carries `"pages": "4"`). With the flag, `total` = the registry's count (positive only), the viewer is not fetched, and GetImage keeps the viewer URL as its Referer. A 4-page document goes 5 -> 4 requests (-20%); across the remaining 17.9M documents about -17.9M of ~156M requests (-11%, ~2 days on a two-exit run). The unknown the A/B decides: whether GetImage serves without a preceding viewer fetch on the same session (the viewer may set a cookie). THE A/B, on a good exit, on login's word: `python "Acris Reproduction.py" --drive <label> --host <name> --lanes documentation:40 --stagger 5 --trust-registry-pages`; read `reqs / pdfs` on the PROGRESS line against the proven ~10.5 for 2005-era documents; if the images serve and the ratio drops by about one, keep the flag; if GetImage answers non-TIFF or the placeholder, the viewer's cookie is load-bearing and the flag is retired. Offline proof: a proof run before it went in (the skip happens only with the flag and a positive registry count; the viewer stays the authority otherwise).
