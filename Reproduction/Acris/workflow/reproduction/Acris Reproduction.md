...
> **The charter, APPROVED 2026-08-28** (the decoder era: `Legal Instruments.db` is the retired SQLite, `fleet.py` the old roster).
...
and log; the fleet launches them as ONE BATCH - one process on one entry, in the cycle's order (one
door at a time only with `--separate`) - and watches them. It relaunches what a relaunch can cure and
never relaunches what a person must decide.

    python "Acris Reproduction.py" --drive OneTouch                     ONE BATCH (login 2026-09-06): identification x5 + registration x5 + documentation x5 in ONE process on ONE entry - one ramp from the first worker to the last (5 s apart), one hang-up, one re-entry from the top, no rate manager
    python "Acris Reproduction.py" --lanes identification:10,registration:20
                                                                       the widths by login's word - Gate 3: identification and registration, 30 workers in one batch (no --drive: no documentation crew)
...
| flag | default | what the fleet does with it |
|---|---|---|
| `--lanes` | `identification:5,registration:5,documentation:5` | LANE:WIDTH,... in launch order (`WIDTHS` in the program) |
| `--mega` | this site's default | every crew in one process through the first lane's `--also` (ONE BATCH) |
| `--separate` | off | one process per lane even on a ONE BATCH site (tests only) |
| `--drive` | | documentation's drive label (the volume label: OneTouch at home, workstation 2's own); required whenever a documentation crew is launched |
| `--fresh-days` | 30 | documentation: a document recorded within this many days with no image is pending, not absent |
| `--edge` | 0 | identification's first start: the last CRFN whose document the table holds; passed on the lane's first launch only - afterwards its edge file is the truth, and a disagreeing `--edge` is refused |
| `--entry-gap` | 20 | seconds between lane launches (and between crews inside a mega lane without ONE BATCH) |
| `--stagger` | the lane's own (5 s) | seconds between worker births inside a lane; passed only when given |
| `--pending-age` | 1 hour | re-check a pending once its last check is this old; passed to every lane (Gate 3 ran `"1 day"`) |
| `--redial-wait` | the lane's own (60 s with the backoff) | seconds of silence before a re-entry; passed only when given |
| `--tries` | the lane's own (4) | re-entries per incident before a lane parks; passed only when given |
...
| `--host` | the machine name | this workstation's name in the cloud |
...
| the cycle, per lane | each lane runs login's cycle on its own session (`rulebook.py`): enter once, births 5 s apart, a closed line redialed by its worker, hang up when the whole width is closed, drop the cut batch, 60 s of silence (×2 refused, ÷2 served), one re-entry on a fresh batch. The fleet passes `--stagger`, `--redial-wait` and `--tries` only when given, so the lanes' own defaults are the one truth; a session close is never the fleet's business - exit 3 comes only after four refused re-entries in a row | login 2026-09-04: batch, enter, stagger, redial until close, exit, rebatch, cycle |
| what each exit means | 0 done · 1 refused to start (another door, parked, arguments): left alone · 2 REFUSED: every lane told to stop, exit 2, a person decides · 3 four re-entries in a row refused: the lane parked itself, never relaunched, a person decides · 4 wall: parked by the lane, left · 5 crash: relaunch after 60 s · 6 drive gone: wait for the drive, relaunch with `--unpark` | `rulebook.py`'s guard could not tell a crash from a refusal (2026-08-30); the drive drop of 2026-09-03 |
...
| cross-station | the same file on workstation 2 with `--drive <label>`; `status` reads the workstation rows of `machinery.updates` (workers, last seen, last word) | rulebook/Rulebook.md |
...
(proven before it went in) shows the acris site launching the mega lane without asking, the hosted crews with
...
The repo era's dated entries, oldest first; sections 1 onward are the decoder-era record (the reading order above).

### §18 — GATE 3: THE FIRST LIVE ONE BATCH — LEVEL (2026-09-06 20:44–21:21)

`python "Acris Reproduction.py" --drive OneTouch --host LoginSurface --lanes identification:10,registration:20 --pending-age "1 day"` → the fleet launched `Acris Identification.py --width 10 --entry-gap 20 --pending-age 1 day --edge 2026000245705 --also registration:20 --one-batch --unpark` (the argv printed before launch; login: "don't freeball the code"). ONE BATCH of 30 on one exit-pool check (one block), entered 20:44:28; the registration crew's births started right after the sync's ramp, 5 s apart, no rate manager. Registration took the 8,876 PROVISIONAL registries first (0009): 0 left by 21:02, every one re-read with its recorded date. Identification LEVEL at 20:54:03: edge 2026000254029, 8,323 new documents, 0 fails, 0 holes (the 09-05 edge 2026000245705 → 254029 = the gap since the old lane's last entry). 20:57:23 the session closed (ACRIS's ~13-minute close on a fresh batch, as on record) → the ONE-BATCH hang-up: the hosted crew left with the host ("the batch hung up - leaving too, 241 of the cut batch dropped"), one wait, re-entry from the top served 20:58:36. The 241 registrations the crew dropped stayed under their 20-minute claims until 21:17–21:20, then landed: registration 21,631,885 / 21,631,885 at 21:21, 0 claims held, 0 empty registries, 0 provisional. ACRIS IS LEVEL on identification and registration; documentation 3,716,943 / 21,631,885 is Gate 4. Richmond level in two minutes (Richmond Reproduction.md §7). The one-batch code behaved as proven offline (proven before it went in): one entry, one ramp, one hang-up, one re-entry.
...
2. **login's sync/registration instinct was right, precisely.** The allowance is shared across the three lanes. The 20:44–21:43 gate-3 **identification+registration ONE BATCH** spent ~27,000 requests (reg 17,227 + sync 10,205) on block **45.95.243**. Documentation entered that same block at 22:15 and got an **instant** notice — the budget was already gone. That explains the 10th notice exactly. It does NOT explain the 11th–15th (documentation alone on fresh blocks, each hitting its own ~25-50k budget). **Going forward this is moot: registration and identification are both at 100%, so only documentation runs and it gets the whole exit to itself.**
...
`Acris Documentation.py` fetches the viewer page once per document only to read `TotalPages`. But the **registry already holds the page count** (`registry["pages"]`, filled at 100% by registration — e.g. the cut doc's registry carried `"pages": "4"`). If GetImage serves without a preceding viewer fetch (unknown — the viewer may set a session cookie GetImage checks), trusting `registry["pages"]` and skipping the viewer cuts **one request per document**: a 4-page doc goes 5->4 requests (20% fewer), directly ~10–25% more docs per exit-allowance and more docs/s. The test: on a good exit, fetch a GetImage for a doc whose viewer was never loaded this session; if it returns the TIFF, the optimization is safe. Proposed behind a `--trust-registry-pages` flag, default off, A/B before trusting. **REJECTED 2026-09-09: the A/B was never run, the flag went into the station's band instead, and registration's count is wrong in BOTH directions — it undercounts (a nine-page document registers as seven, so the pdf lands short) and overcounts (2003012101793002 registers 39 where the viewer says 3). 39,360 pdfs deleted. The idea is closed, not pending: the viewer is the only page authority.**
...
**02:09:14 — LOGIN'S "ONE LAST CHANCE": THE GOLDEN LINE ON A FRESH EXIT, 140.99.118 — HOLDING AT 30 MIN / 85,000 REQUESTS.** login: "one last ACRIS chance using old code based on the golden run; if it blocks, gate 5." The runbook reverted to the golden line (`--pdf-workers 80`, the rate manager's own defaults: floor 5 / ideal 6-7 / hard 8, 60 req/s ceiling, width 20..120, session 1M, stagger 5) and launched through `doc_lane.py launch` exactly as the golden nights: lane + supervisor + helpers, the lane's park cleared, five draws one block 140.99.118 (the seventh block of the night, fresh). The manager ramped 1 -> 46 -> 56, trimmed to 53 as 51-53 lines projected 95% of the 60 req/s ceiling at this exit's speed, and holds there: PROGRESS 29m - 84,579 requests at 48.6/s - 8,297 pdfs - 4.5-5.4 docs/s - 52-53 kept connections - zero events. That is 3x the best fresh block of the night (89.106.14, 49k) and past the 60k at which 45.130.203 was cut on 09-05, on the same old binary refused at 26k two hours earlier on 173.244.43. So the exit's allowance is the variable, and this exit has a large one; the golden line does what it did. The Supabase board does not move for this run (the old lane feeds the old store/SQLite); the carry-over into the cloud is the morning's job. Gate 5 waits: it starts only if this run is cut.

**02:42:54 — THE GOLDEN LINE CUT AT 33.7 MIN / 96,756 REQUESTS: DEAD TRANSPORT, THE EXIT UNCHANGED.** The lane's own breaker: 58 transport errors in 60 s on the pdf floor, every kept line to 157.188.15.133 gone between block_watch's 02:42:26 tick (57 ESTABLISHED) and its 02:42:56 tick (0 ESTABLISHED, 4 TIME_WAIT). The exit did NOT move: draws 140.99.118.79 at 02:32, .87 at 02:42:41, .46 at 02:42:59 - the same block before, during and after the cut, so this was not a Lightway reconnect (which drops every socket on the machine) but the far side hanging up on this exit. Run end: 33.7 min, 96,756 requests at 47.9/s, 9,450 pdfs, 49 imageless, 13 fail. That is the largest allowance of the night by 2x (89.106.14 49k, three others 24-28k) and the second shape of a block on the ledger (the notice page, or the lines cut); which one it is, the supervisor's re-entry says: it waits for 60 s of silence and zero lines, then enters ONCE, staggered - a notice at that entry = the exit's allowance spent (~97k on this block); a run = a transient cut. No probe by hand; the runbook's re-entry is the detector.

**02:45:30 — THE RE-ENTRY RAN: THE CUT WAS A LINE DROP, NOT A BLOCK.** The supervisor's one staggered entry on the same block (140.99.118.5, batch from above 2005081800719001, try 1/4) met no notice: request 1 served, the ramp climbed one worker every 5 s, and at 3 min the run read 3,847 requests at 21.4/s, 312 pdfs, 36 workers, 0 fail, 37 kept lines. So the 02:42:54 hang-up is the third shape on the ledger - the far side dropping every line at once and serving again a minute later - and this exit's allowance was NOT spent at 97k. What the ledger now says about 140.99.118: 33.7 min / 96,756 requests clean, one line drop, re-entry served. The golden line + the supervisor's re-entry rule (60 s of silence, zero lines, ONE entry, four tries per incident) handled it without a hand on it; that is the runbook doing its job and the reason a person never redials by hand. Gate 5 still waits.

**03:21 — THE SECOND RUN OUTLASTS THE FIRST; THE BLOCK PASSES 189,000 REQUESTS.** Minute 35 of the re-entry: 92,187 requests at 43.9/s, 8,721 pdfs, 49 workers held on the door curve (52 bought nothing), 8 fail, 49 kept lines + 5 CLOSE_WAIT, exit 140.99.118.203 - the same block since 02:09. Two runs on this block: 96,756 + 92,187 = 188,943 requests, 18,171 pdfs, one line drop between them. That is past every non-golden allowance on the ledger (157k on 09-05's fresh block, 127k on 09-04's small day, 24-60k on the week's others) and the pace sits at the exit's own speed, 4.5-5.1 docs/s. The lever list stands as written: the allowance is the exit's, the golden line runs it out at ~45-48 req/s, and the only code lever left is fewer requests per document.

**04:03 — 300,000 REQUESTS ON ONE BLOCK TONIGHT.** Minute 77 of the re-entry: 203,556 requests at 44.0/s, 18,979 pdfs, 49 workers held on the door curve, 31 fail / 8 short (the shorts flat since 03:42), 49 kept lines + 6 CLOSE_WAIT, exit 140.99.118.5. The block's night: 96,756 + 203,556 = 300,312 requests, 28,429 pdfs, one line drop, zero notices, since 02:09. The second run alone (203k) is past the 157k of 09-05's fresh block; the block is at a quarter of the golden 1.35M and still serving at the exit's own speed (~44 req/s, 4.0-4.5 docs/s over the run).

**04:41 — THE SEVENTEENTH NOTICE: 140.99.118 SPENT AT 415,321 REQUESTS.** REFUSED at 2005090600919001 (5/5 notice signals, 'further access to acris is denied'), the lane self-parked, the supervisor: 'not mine to relaunch; the notice lifts on its own clock, a person clears the park' (04:42:17). Run end 116.4 min - 318,565 requests at 45.6/s - 29,291 pdfs - 115 imageless - 47 fail - 8 short - 56 workers. The block's night, 02:09 -> 04:41: two runs, 96,756 + 318,565 = 415,321 requests, 38,741 pdfs, one line drop, one notice. Zero lines to ACRIS after the park; the exit unchanged (140.99.118.21 at 04:42). So the allowance of this exit was ~415k - the low end of the golden band (480k-1.35M), 15x the week's small blocks, on the golden line with the manager - which settles the question login put to the night: the same code, the same wire, the same cadence; the exit's allowance is the whole variable. login's rule for a block: Gate 5 (the One Touch cleanup list + doc_id -> identifier), lanes and boards paused; the morning decides the exits. The park stays; nothing re-enters this block.

**04:50-05:10 — GATE 5, THE IDENTIFIER: DONE AND PROVEN (commit ede022f).** With the lane parked on the seventeenth notice, 0015 renamed the cell `doc_id` -> `identifier` in `acris`, `richmond` and `machinery.claims` (applied 04:56:45 in 9 s; the first push rolled back because a `create or replace` may not drop a function's defaults - claim 500 / 20 min, land 1 h - kept, it applied), re-created the four field/parcel views and re-stated claim()/land(). The code names the column by its name in 30 files - cloud.py's SQL and the land payload key `{identifier, value}` (the Outbox still reads the old key from lines written before), the lanes' payload reads, the mover's SQL (its own moves/rejects logs keep `doc_id`), the tests, and the vocabulary in the md. Registry DATA keys (`references[].doc_id`, enumeration's journal) and Python identifiers stay. Proofs: eight offline tests ALL OK, test_schema ALL OK on the populated table (04:57-05:06), cloud.py's reads against the renamed table, `acris_update` ticking at 05:07 with the boards never stopped (they name only `reproduction.updates`). The One Touch half of gate 5 is a list, not an act: the inventory (6.06M files / 3.08 TB in the tree; Ignore 663 GB of staged copies; the recycle bin 43 GB; _legacy-pages 2.2 GB; legal-instruments empty) is login's to clear.

**05:42 — THE EIGHTEENTH NOTICE, AND THE UNIT IS THE PROVIDER, NOT THE BLOCK.** login: "try again on a new exit that I give you now" - five draws one block 147.90.160, never on the ledger; the park cleared on that word; `doc_lane.py launch` at 05:42:14 (dead window clean, exit pool one block). REFUSED at 2003030501723001 at request 2 - the first document's viewer fetch - run end 0.1 min. Then ONE diagnostic request from a different client on the same exit (curl: the lane's UA and Referer, no cookies, curl's own TLS) to the viewer URL of 2005081702043001: HTTP 307 'Document Moved' to `https://a836-acris.nyc.gov/BandwidthPolicy/ACRIS-BW-POL.html` - the notice, for anyone on this exit. So the refusal is the address's, not our lane's key. The owners (ipinfo, no ACRIS request): 147.90.160 = AS137409 GSL Networks (Phoenix) - and so are 140.99.118 (Singapore; tonight's 415k), 173.244.43 (Dallas; the 26k refusal), 89.106.14 (Singapore; 49k), 173.239.217 (Amsterdam; the 9th) and 45.130.203 (Frankfurt; the 09-05 60k). 94.20.154 (Baku, the 11th's 24k) is Delta Telecom AS29049; 45.95.243 (Vienna, the 10th) is Datacamp AS212238. The reading that fits every entry on the ledger: ACRIS keeps its allowance per PROVIDER (the ASN, or its whole address list), shared by every block of it - 140.99.118's 415k spent GSL, and a GSL block nobody had used was refused at its first request an hour later; the 09-06 'fresh exits refused at request 1' were GSL after GSL. Locations are cosmetic; the ASN is the door. What follows: pick exits by owner (five draws + ipinfo before an entry; never GSL, Datacamp or Delta until one of them is shown to lift), one entry per fresh provider; the office residential line is a fresh provider by construction; whether an allowance renews = ONE request on any block of a spent provider after a rest (GSL at 16:41 would be 12 h), on login's word.

**05:53-06:05 — WHAT TRIGGERS IT: THE RANGE'S STANDING, NOT THE CLIENT (login: 'figure out what is triggering it').** Three controlled facts in one hour, the client held constant (the same binary, the same Python TLS, the same Chrome/128 UA and Referer, the same cookie handling): (1) 'Bosnia' in the app = 200.162.147 = AS62240 Clouvider, Lelystad NL, a provider never on the ledger - REFUSED at request 1 (the 19th notice, 05:53). (2) 'Brazil' = 2.57.171 = AS262287 Latitude.sh, Sao Paulo, hosting and proxy-flagged by ip-api - SERVED: one lane-style request at 06:03 answered the viewer page, TotalPages 11; the golden line entered 06:04:54 and read 488 requests / 14 pdfs / 13 lines at minute 1. (3) Earlier, on 147.90.160 ('Bolivia' = GSL Phoenix), the lane and a curl with a different TLS stack got the same 307 to the policy page. With the 415k the identical client got on 140.99.118 at 02:09, that rules the client out: TLS fingerprint, UA, cookies and header shape were the same across served and refused; only the exit changed. login's hypothesis of a client-side signature is falsified by these contrasts. The trigger has two mechanisms: a range ACRIS already refuses answers the notice at request 1 (its standing, from our spend or others' - 147.90.160 after GSL's 415k; 200.162.147 with no history of ours); a range in good standing serves until a cumulative allowance - 24k to 2.3M by range - and then the notice. ACRIS's policy page (25,103 bytes, the notice itself) says both in its own words: 'detection of automated scripts/robots that are capturing data' and 'having exceeded the bandwidth limits', and offers the City Register's subscription data services (212-487-6300) for large amounts of data. Unknown still: whether a spent range renews (one request after a rest, on login's word) and a range's allowance before it is spent. The app's location names are cosmetic; the exit's owner is the door; `doc_lane.py exit` reads it before an entry.

**07:00 — LATITUDE.SH ('BRAZIL', 2.57.171) PAST 157,000 AT THE HOUR.** Minute 55: 171,405 requests at 51.9/s, 15,460 pdfs, 39 workers held on the ceiling line, 4.6-5.5 docs/s, 21 fail, 318 imageless, 39 kept lines + 4 CLOSE_WAIT, exit 2.57.171.39 - one block since 06:04:54, no event. Past the 157k of 09-05's fresh block; the second-largest standing on the ledger after Singapore's 415k, and a shade faster (52 vs 45 req/s). The survey protocol did its first job here: `exit` said fresh owner, one lane-style request said served, the golden line entered.

**07:33 — THE TWENTIETH NOTICE: LATITUDE.SH ('BRAZIL', 2.57.171) SPENT AT 277,929 REQUESTS.** REFUSED at 2005092000319001 (5/5 signals) at minute 88, self-parked, the supervisor honors (07:33:28). Run end 88.3 min - 277,929 requests at 52.4/s - 25,543 pdfs - 1,280 imageless - 34 fail - 39 workers, one block from 06:04:54 to 07:33, no line drop. The night's two big standings: Singapore (GSL) 415,321 / 38,741 pdfs; Sao Paulo (Latitude.sh) 277,929 / 25,543 pdfs - 64,284 pdfs in the old store since 02:09, at 45-52 req/s, the same line. The allowance per range on the ledger now reads 24k, 26k, 49k, 60k, 157k, 278k, 415k, 1.35M, 2.3M - a range's standing is its own; nothing in our behaviour separates them (same code, rate, cadence). The lever stays: fresh owners in good standing, found by `exit` + one request; the office residential line; and the City Register's subscription route.

**2026-09-07 12:2x-13:3x — THE CARRY-OVER, THE ORGANIZATION, 0016 + 0017.** The two overnight runs of the old lane (the golden line, 02:09-04:5x on 140.99.118, and the earlier one) had written 63,341 pdfs into the OLD store and the old SQLite, not the cloud (login: "there were 2 successful runs that need to go into supabase"). A carry-over program outside the repo (`C:\dev\cre-office\carry_over.py`) walked the old store's 2005 folder, asked the cloud which of those documents still had an empty cell, checked each file whole (`%PDF-` ... `%%EOF`), moved it to its canonical address in the One Touch tree (the same day-folder rule) and landed the paths through `reproduction.land()` as the documentation lane from LoginSurface: 52,622 landed (36,242 in the first pass, 16,380 in the second after a network drop at 12:57), 10,719 were re-fetches of documents the cloud already held (the old lane's to-do list was the old SQLite's, not the cloud's), 0 not whole, 0 unknown to the table; nothing deleted, every move logged. Documentation 3,726,582 -> 3,779,204 (17.47 %). From here only the GitHub lane writes documents, straight to the cloud and the tree. Then the repo was organized to login's tree (every folder one md and one py, the machinery as one `rulebook.py`, the schema as one `supabase/schema.sql`, no tests, synchronization -> identification everywhere) and the cloud with it: 0016 (three schemas: reproduction / machinery / reading) and 0017 (the lane row `identification`; the boards in three blocks of four with source before lane). Both boards restarted on the new code 13:22:55.

### 2026-09-07 — THE AUDIT OF THE SOURCE FOLDER AGAINST THE CODE

The fourteen files of the source folder audited against the code (41 findings: the words lagging the code and the code lagging the fold) and fixed. In this file: the opening names the program, ONE BATCH and the table; section 0 says ONE BATCH (not one door at a time) and carries the fleet's own flags from `build_parser`; the reading-order note's names restored (`fleet.py`, `rc_lane.py` - a global rename had rewritten the record); the documentation-lane runbook's headings sit under its dated heading; the repo-era entries (section 18, the overnight verdict) collected here, oldest first. In the program: the docstring says ONE BATCH and points at section 0, `identification` on the Gate 3 line, the path line names `rulebook.py` as one module, the band comments stand in time order with the dict's knobs last (floor 4 / goal 5-6 / ceiling 6 docs/s, width 20..60, step 5, login 2026-09-07 01:2x), and the five wrappers nothing called (`parse_lanes`, `Fleet`, `status`, `stop`, `width`) left; `LANES`, `WIDTHS`, `MANAGE` and the site are unchanged.
...
**identification** — the MONITOR watches the crfn edge every 10 s,
...
table `update_floors`: identification · registration · documentation per
...
`ACRIS Live Sync 4AM` (routine_identification.py) and
...
| `identification\|acris` | the **main** `update_board` row (internal key `identification`, displayed `reproduction`) |
...
both. ⚠ Parking `identification|acris` does NOT touch the
identification FLOOR row: that one is written separately and hardcoded
...
- **NOT identification.** No crfn walk, no monitor.
...
`CRE Ledger Refresh 4AM` task (`routine_identification --source richmond`,
...
**15:03:13 HOME 1x40 PAUSED for the restructure** (login: pause any reproduction lanes given the massive restructure): 55 min, 16,134 pdfs, 260 imageless, 21 fails, 57.2 req/s; log preserved as acris_repro_document.log.20260902-1503-paused-for-restructure; name parked. Dead ends cleaned: the office-transfer bucket in the OLD project and the OneDrive CRE Office folder. Next: login walks the restructure step by step - db, then the four lanes (enumeration=audit, identification, registration, documentation) with the status rules built into their code, then the monitor with a tab per lane.
...
- 09-01 04:00  `_routine_identification_run.log`  notices/refusals: 2
