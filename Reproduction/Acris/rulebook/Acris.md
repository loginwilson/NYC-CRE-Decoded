...
Every URL is minted from the id. No URL, token or key is ever stored: the table holds `source`, `identifier`, `registry`, `document`, nothing else - not even a clock: the wait before a pending's next check is its claim (0004) (`../../rulebook/Rulebook.md`).
