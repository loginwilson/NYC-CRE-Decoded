...
TAG = "credoor"
LOG = os.path.join(HERE, "do_doors.log")
...
REST_MAX_MIN = 60
LEDGER = os.path.join(HERE, "cloud_doors.json")     # the kept doors: name, id, region, ip, block (/24), keeper port, kept_at
...
REGIONS = ("syd1,sgp1,ams3,lon1,blr1,fra1,tor1,nyc1,nyc3,nyc2,sfo2,sfo3,atl1,ric1,mkc1,mem1")
...
SPEED_SECONDS = 12
PROBE_PORT = 1100                                   # scratch ports for one-request probes; the keepers live on 1080..1099
...
def api(method, path, body=None):
    ...
    req = urllib.request.Request("https://api.digitalocean.com/v2/" + path, data=data, method=method,
    ...
    try:
        ...
        raise SystemExit("DO API %s %s -> %s %s" % (method, path, e.code, e.read().decode()[:200]))
...
def cmd_create(a):
    ...
    body = {"names": names, "region": a.region, "size": SIZE, "image": IMAGE, "ssh_keys": [kid], "tags": [TAG]}
    if a.count == 1:
        body = {"name": names[0], "region": a.region, "size": SIZE, "image": IMAGE, "ssh_keys": [kid], "tags": [TAG]}
    res = api("POST", "droplets", body)
    made = res.get("droplets") or [res.get("droplet")]
    for d in made:
...
def _wait(ids, timeout=420):
    ...
    while time.time() - t0 < timeout and len(ready) < len(ids):
        for i in ids:
            if i in ready:
                continue
            d = api("GET", "droplets/%s" % i)["droplet"]
            ip = public_ip(d)
...
def cmd_map(a):
    ...
    for r in regions:
        ...
        d = api("POST", "droplets", body)["droplet"]
...
def _free_port(taken):
    for p in range(1080, 1100):
        if p not in taken and not _listening(p):
            return p
    raise SystemExit("no free keeper port in 1080..1099")
...
def _created_total():
    ...
    p = os.path.join(HERE, "do_doors.log")
    try:
...
def _fill_round(a):
    ...
    random.Random(int(time.time()) // 60).shuffle(shuffled)
    kid = key_id()
    limit = api("GET", "account")["account"].get("droplet_limit", 10)
    alive = droplets()
    ...
    made = []
    bad = set()
    bad_why = set()                     # regions DigitalOcean refused THIS round: tried once, then stepped over
    tries = 0
    while len(made) < n and tries < len(regions) * 2:
        r = shuffled[(len(made) + tries) % len(shuffled)]
        tries += 1                                                # (14:0x: seven tor1 blocks refused in a row while nyc/sfo/ams went untried)
        if r in bad:
            continue
        k = 1
        ...
        name = "door-%s-%d" % (r, k)
        try:
            d = api("POST", "droplets", {"name": name, "region": r, "size": SIZE, "image": IMAGE, "ssh_keys": [kid], "tags": [TAG]})["droplet"]
        except (Exception, SystemExit) as e:            # api() raises SystemExit on an HTTP error - Exception alone never catches it
            # A REGION WE CANNOT BUILD IN IS NEWS, NOT A FAILURE: nothing was created, so nothing is billed - step over
            # it (a sold-out region or a raised limit answers here) and keep the round alive.
            bad.add(r)
    ...
    for _v in _verdicts:
        ...
        if b in spent:                                    # NOTHING RE-ENTERS A BURNED ADDRESS (login's rule): destroyed
            api("DELETE", "droplets/%s" % i)              # without spending a probe request on a block we know is done
...
def cmd_adopt(a):
    ...
    for d in alive:
        ...
        b = block_of(ip)
        if b in held:
            api("DELETE", "droplets/%s" % i); log("adopt burnt %s %s %s: block %s already held" % (i, name, ip, b))
        ...
        if v != "SERVED":
            api("DELETE", "droplets/%s" % i); log("adopt burnt %s %s %s: %s" % (i, name, ip, v))
        ...
        if speed is not None and speed < MIN_SPEED:
            api("DELETE", "droplets/%s" % i); spent_write(b, "only %.2f docs/s" % speed)
