...
The cycle's authority is `../reproduction/Richmond Reproduction.md`; the shared machinery (one entry, staggered births, the outbox, the heartbeat, the refusal park, the hang-up and wall breakers, the width control, the lock) is `Reproduction/lane.py` and is not repeated here.
...
- **The edge** (`registration.edge.json`) is the last day whose registries were walked. It moves only after a window's pages have all answered and its details are all landed, never on an empty-looking page. The first start must name it (`--edge`); the file refuses an `--edge` that disagrees with it (remove the file if the day is meant to change).
...
`0` stopped · `2` refused (parked; `--unpark` after a person decides) · `3` redials exhausted or the probe broken · `4` wall · `5` crash.
