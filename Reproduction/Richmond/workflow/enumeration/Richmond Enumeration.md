...
Any workstation can run it; the ledger of a census lives beside the file on the workstation that swept it. Exit codes: 0 the difference is 0 · 1 a difference · 7 unproven · 2 refused · 3 the probe is broken, or the wire died in the trailing window or a range (in the census a dead wire leaves its window unswept and the run leaves with 7) · 5 crash.

This program takes its own flags, not the shared ones: it builds its own parser and never calls `rulebook.add_common_args`, so `--width`, `--drive`, `--fresh-days`, `--claim`, `--ttl`, `--limit`, `--log` and the managers' knobs are refused at the command line. Its flags, from its own help text: `--days` (30, the trailing window checked live, clamped to the county's 30-day cap) · `--from` / `--to` (a date range instead of the trailing window, `--to` defaulting to today) · `--all` (the census: every window from 1850 to today, resumable) · `--workers` (10, the windows swept at once in the census) · `--pace` (0.3, seconds between the pages of one window) · `--unpark` (run although the county declined last time: a person has decided) · `--host` (this workstation's name; default the machine name).
...
2026-09-07 — the audit against the code: the shared-flags sentence, copied from the cycle lanes, replaced with this program's own flags (it never calls `add_common_args`, so the shared ones are refused); exit 3's rule line says where a dead wire ends the run and where it leaves a window unswept for 7, as the two entries above recorded; the program's `County` no longer takes a `rep` it never read; the history in date order.
