...
lane (its own landed count, rate, workers, last seen, last word).  What a person opens is reproduction.richmond_update
...
This file's own authority is Richmond Update.md beside it; the shared rules are in ../../rulebook/rulebook.py.
...
def main():
    ap = argparse.ArgumentParser(description="richmond update: the board - reads machinery.updates, writes the rates, eta and status back")
