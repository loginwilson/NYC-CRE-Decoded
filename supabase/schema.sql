...
comment on column reproduction.acris.identifier is 'filled by synchronization (the ACRIS document id; every URL is minted from it)';
...
comment on column reproduction.richmond.identifier is 'filled by synchronization (the Richmond County internal id; every URL is minted from it)';
...
$function$;
comment on function reproduction.reconcile(p_source text) is 'recount landed and needed for the phase and the three lanes from the indexes and overwrite the totals rows; on demand - after loads and hand edits, never on the tick';
