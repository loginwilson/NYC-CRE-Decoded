...
| 2026-09-07 14:29 | a hand edit, logged in supabase.log: workstation 1 seeded to the totals in `machinery.updates` (all four lanes, both sources) - a workstation row is that machine's total, all time, and everything before the second workstation was this machine's (login); the workstation rows sum to the total from here |
...
## 0018 - THE BOARD'S ORDER AS THE TABLE EDITOR SHOWS IT (2026-09-07 13:4x)

login, opening `acris_update` after 0017: "That doesn't look good" - the rows came out scrambled (the two spacers first, then the twelve acris rows in no order). Measured: the Table Editor does not show a view in the view's own order; it sorts by the first column ascending, and `select ... order by source limit 100` reproduces the screen exactly (the spacers' empty source first, the ties in quicksort's order). When every row carries the same first value the sort has nothing to reorder and the view's order comes through (measured: fourteen rows in dictated order). So the spacer rows carry the source too; the totals are named as login named them (`reproduction total` ... `documentation total`); a workstation block nobody has claimed reads `pending` (login: "For acris we are running 2 workstations. Richmond likely will never need it but might as well have the second one 'pending' in case we ever had to").

## 0019 - THE SPACER ROWS BLANK (2026-09-07 14:0x)
